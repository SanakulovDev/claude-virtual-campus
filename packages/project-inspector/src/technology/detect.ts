...
import type { DetectedTechnology, ProjectTechnologyProfile } from '@campus/contracts';
import { MARKER_RULES } from './markers';

const MAX_CONFIG_FILE_BYTES = 64 * 1024;

async function safeReadSmallFile(dir: string, filename: string): Promise<string | null> {
  try {
    const filePath = path.join(dir, filename);
    const handle = await readFile(filePath, { encoding: 'utf8', flag: 'r' });
 ...
 */
export async function detectTechnologyProfile(dir: string): Promise<ProjectTechnologyProfile> {
  let entries: string[] = [];
  ...
  async function contentOf(filename: string): Promise<string | null> {
