...
/**
 * Stable ordering of a studio's agents so desk assignment never shifts: main agents first,
...
export function assignDesks(agents: AgentRow[]): Array<{ agent: AgentRow; deskIndex: number }> {
  const ordered = [...agents].sort((a, b) => {
    const aMain = a.externalAgentId === 'main-claude' || a.externalAgentId === 'main-codex' ? 0 : 1;
    const bMain = b.externalAgentId === 'main-claude' || b.externalAgentId === 'main-codex' ? 0 : 1;
