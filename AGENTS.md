...
Guidance for anyone (human or Codex) working in this repository.

## Product goal

Codex Virtual Campus visualizes real Codex activity across an arbitrary number
...
- Technology detection (`packages/project-inspector`) enriches the experience; it must
  ...
  room, a main-Codex agent, and full event handling.
- The installer (`packages/Codex-plugin/installer`) only ever touches `.Codex/` inside
...
- Payload size limits (512kb) on `/api/Codex/events` and `/api/Codex/approval`.
...
`POST /api/Codex/events` -> validate (zod) -> resolve project (`project-inspector`) ->
...
Codex's officially documented hook events (as of when this was built) are:
...
present. Verify this mapping against your installed Codex CLI's own docs before
